...
def plot_peak_histograms(df, label, output_path):
    ...
    if day_buy is not None and len(day_buy) == 24:
        bar_colors = ["#e74c3c" if _is_peak_hour(h) else "#4a90d9" for h in range(24)]
        ax.plot(range(24), day_buy, "o-", color="#333333", lw=2, markersize=4, zorder=3)
        # Shade peak period backgrounds
        for h0, h1 in spans:
            ax.axvspan(h0 - 0.5, h1 - 0.5, alpha=0.15, color="red", zorder=1)
        ax.axvline(peak_idx % 24, color="#e74c3c", ls="--", alpha=0.7,
