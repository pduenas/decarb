...
Outputs go to:  ERCOT_ROOT/out/decarb_results/elec_all_flat/
"""
...
CASE_TYPE        = "tou_cap"
CASE_0           = "elec_0_tou_cap_results"
CASE_100         = "elec_100_tou_cap_results"

DECARB_IN_ROOT   = os.path.join(ERCOT_ROOT, "in",  "decarb_inputs_alt")
DECARB_OUT_ROOT  = os.path.join(ERCOT_ROOT, "out", "decarb_results")
METADATA_PARQUET = os.path.join(ERCOT_ROOT, "in",  "TX_upgrade0.parquet")
OUTPUT_DIR       = os.path.join(DECARB_OUT_ROOT, "elec_all_" + CASE_TYPE)
...
# GenX output config
WRITE_GENX_INPUTS = True   # set True to write Demand_data.csv files for GenX
...
_HOUR_ZERO = pd.Timestamp("2018-01-01 00:00")

def _hour_to_datetime(hour_idx):
    return (_HOUR_ZERO + pd.Timedelta(hours=int(hour_idx))).strftime("%Y-%m-%d %H:%M")


def compute_summary(buy_agg, heat_agg, cool_agg, tariff_rates, pct_label):
    """Compute peak, total consumption, total cost, and peak event datetimes."""
    peak_buy_idx  = int(np.argmax(buy_agg))
    peak_heat_idx = int(np.argmax(heat_agg))
    peak_cool_idx = int(np.argmax(cool_agg))

    n          = min(len(buy_agg), len(tariff_rates))
    total_cost = float(np.dot(buy_agg[:n] * 1000.0, tariff_rates[:n]))
    return {
        "electrification_pct":   pct_label,
        "peak_buy":              float(buy_agg[peak_buy_idx]),
        "peak_buy_datetime":     _hour_to_datetime(peak_buy_idx),
        "peak_heating":          float(heat_agg[peak_heat_idx]),
        "peak_heating_datetime": _hour_to_datetime(peak_heat_idx),
        "peak_cooling":          float(cool_agg[peak_cool_idx]),
        "peak_cooling_datetime": _hour_to_datetime(peak_cool_idx),
...
def plot_stacked_loads(all_buy, all_heat, all_cool, output_dir):
    ...
    for title, data_dict, filename in datasets:
        ...
        ax.set_title("Daily Peaks of Electricity Demand in Texas by Heating Electrification Level - Flat Tariff with CC",
                     fontsize=16, fontweight="bold", pad=14)
        ax.set_xlabel("Day of Year", fontsize=14)
        ax.set_ylabel("Daily Peak Electricity Demand [GW]", fontsize=14)
        ...
        if all(lbl in data_dict for lbl in endpoint_labels):
            ...
            ax2.set_title("Daily Peaks of Electricity Demand in Texas by Heating Electrification Level - Flat Tariff with CC",
                         fontsize=16, fontweight="bold", pad=14)
...
if __name__ == "__main__":
    ...
    print(f"\n{'=' * 70}")
    print(f"  Electrification Interpolation — elec_all_{CASE_TYPE}")
    print(f"{'=' * 70}")
    ...
    for pct in all_levels:
        ...
        if WRITE_GENX_INPUTS:
            write_genx_demand(buy_agg, pct_label, case_suffix=f"_{CASE_TYPE}")
