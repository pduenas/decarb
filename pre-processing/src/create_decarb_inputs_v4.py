...
def generate_tm_data(consumption_df, setpoint_df, building_row):
    ...
    # Step 10: Total energy (kept as fallback)
    total_cols = [
    ...
    # Step 11: Equipment power (pQequip) — directly from plug loads
    ...
    if plug_col in consumption_df.columns:
        ...
        # Fallback: total minus known end uses
...
def create_sp_file(output_dir: Path, template: pd.DataFrame,
    ...
    if "Shared Heating" in heating_id:
        ...
        # Heating-only: pHVmx > 0 AND pACmx == 0 (no heat pumps), scoped to fuel
    ...
    if cooling_shared:
        # Cooling-only: pACmx > 0 AND pHVmx == 0 (no heat pumps)
        pool = hvac_df[
            (hvac_df["ty"].notna()) &
            (hvac_df["ty"].astype(str).str.lower() != "none") &
            (hvac_df["pACmx"] > 0) &
...
def process_building(bldg_id: int) -> dict:
    ...
    try:
        ...
        # Build chp/hvac dfs only when needed (for saving or for sp.csv)
        chp_df  = None
        hvac_df = None
        if RUN_CHP or RUN_SP:
