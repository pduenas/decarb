...
def get_capacity_at_design_temp(
    ...
    if heating_series.isna().all() or heating_series.max() == 0:
        ...
        outdoor_temp_f = consumption_df["out.outdoor_air_drybulb_temp..c"].mean() * 1.8 + 32
    ...
    return result, peak_load_btu_hr, outdoor_temp_f
...
def find_multi_unit_options(
    ...
    """
    Try to find heat pump combinations that together meet the peak load,
    by installing N identical units of the same model.

    For each N in [2, 3, 4]:
      - N identical units of the same model must together meet peak_load_btu_hr
      - i.e. each unit must individually meet peak_load_btu_hr / N

    Parameters
    ----------
    neep_df : pd.DataFrame
        Full NEEP dataframe with capacity_btu column.
    peak_load_btu_hr : float
    ...
    """
    for n in [2, 3, 4]:
        capacity_per_unit = peak_load_btu_hr / n
        eligible = neep_df[neep_df["capacity_btu"] >= capacity_per_unit].copy()
...
if __name__ == "__main__":
    ...
    print(f"Found {len(consumption_files)} buildings to process.")

    zero_hp_match = []
    summary_rows = []

    for filepath in consumption_files:
        ...
        try:
            ...
            neep_df, peak_load_btu_hr, outdoor_temp_f_val = get_capacity_at_design_temp(neep_base, consumption_df, conditioned_area_sqft=building["conditioned_area_sqft"], verbose=False)
            ...
            if n_sufficient == 0:
                ...
                neep_df, number_hp = find_multi_unit_options(neep_df, peak_load_btu_hr)
