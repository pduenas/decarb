...
def create_building_timeseries(
    ...
    Weather data (solar radiation, wind speed, humidity) is read directly
    from the consumption parquet files using the following columns:
    ...
    Args:
        ...
        ts_config: Dict of column name keys for temperature and load
    ...
    # Column names for weather data embedded in consumption files
    ...
    for _, row in merged_bldg_df.iterrows():
        ...
        try:
            ...
            # ------------------------------------------------------------------
            # Extract weather columns from the consumption file
            # ------------------------------------------------------------------
            ...
            if not has_solar and not skip_missing_weather:
                # Solar radiation is the minimum required weather feature
                continue
            ...
            weather_src_cols = []
            if has_solar:
                weather_src_cols.append(SOLAR_COL)
            if has_wind:
                weather_src_cols.append(WIND_COL)
            if has_humidity:
                weather_src_cols.append(HUMIDITY_COL)

            df_bldg = df_bldg[base_cols + weather_src_cols].copy()
            ...
            # Convert solar radiation from W/m² to W (total over floor area)
            if has_solar:
        ...
        if save_outputs:
            ...
            df_bldg.to_csv(out_path, index=False)
...
def compute_regression_coefficients(
    ...
    Results saved to: out/thermal_model/<feature_set>/regression_coeff_<feature_set>.csv
...
# ============================================================
# Worker function (runs in a subprocess for concurrent mode)
# ============================================================
def _worker(args):
    """
    Top-level worker function for ProcessPoolExecutor.
    Processes a single batch of buildings: builds timeseries, runs regression
    for all feature sets, and writes results to per-batch temp parquet files.

    Args:
        args: tuple of (batch_idx, batch_ids, input_dir, output_dir,
                        ts_config, feature_sets, save_outputs,
                        skip_missing_weather, tmp_dir)

    Returns:
        dict mapping feature_set name -> number of buildings saved, or
        an error string if the batch failed.
    """
...
# ============================================================
# Function 3: Run pipeline (concurrent, sequential batch, or single pass)
# ============================================================
def run_pipeline(
    ...
    buildings in one of three modes controlled by the batch flag:

      batch=False  — single pass: all buildings processed at once, results
                     saved at the end.

      batch=True   — concurrent: buildings split into chunks of batch_size,
                     up to n_workers chunks processed simultaneously using
                     ProcessPoolExecutor. Each worker writes its results to
                     a temp parquet file; the main process merges all temp
                     files into the final CSV + parquet once all workers finish.

    Output saved to: out/thermal_model/<feature_set>/
    ...
    Args:
        ...
        skip_missing_weather: If True, keeps buildings with no weather data
        batch: If True, run concurrently with ProcessPoolExecutor
        batch_size: Buildings per worker batch (default 100)
        n_workers: Max parallel worker processes (default 24)
    """
    ...
    for feature_name, feature_cols in feature_sets.items():
        out_dir = output_dir / "thermal_model" / f"{feature_name}"
    ...
    if batch:
        # Split building IDs into batches
        batches = [
            bldg_ids_list[i:i + batch_size]
            for i in range(0, total, batch_size)
        ]
        n_batches = len(batches)

        # Temp directory for per-batch parquet files
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=n_workers) as executor:
            futures = {
                executor.submit(_worker, args): args[0]  # batch_idx as key
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                if isinstance(result, str):
                    ...
                    t_elapsed = time.time() - t_start_workers
                    print(f"  [{completed}/{n_batches}] Batch {batch_idx} done "
                          f"({t_elapsed:.0f}s elapsed) | {counts_str}")
        ...
        # ----------------------------------------------------------
        # Merge all temp parquet files into final outputs
        # ----------------------------------------------------------
        ...
        for feature_name in feature_sets:
            ...
            # Clean up temp files for this feature set
            for f in tmp_files:
                f.unlink()
        ...
        for feature_name, feature_cols in feature_sets.items():
            ...
            out_dir = output_dir / "thermal_model" / f"{feature_name}_v4"
            out_csv = out_dir / f"regression_coeff_{feature_name}.csv"
            out_parquet = out_dir / f"regression_coeff_{feature_name}.parquet"
...
def plot_lowest_r2_buildings(
    ...
    coeff_file = output_dir / "thermal_model" / f"{feature_set}" / f"regression_coeff_{feature_set}.csv"
    ...
    metadata_file = output_dir / "thermal_model" / f"{feature_set}" / f"regression_coeff_{feature_set}_metadata.json"
    ...
    plots_folder = output_dir / "thermal_model" / f"{feature_set}" / "plots"
...
if __name__ == "__main__":
    ...
    batch_size   = 100   # Buildings per worker batch
    n_workers    = 24    # Number of parallel worker processes
