...
PEAK_HOURS = {
    "elec_0_flat_results":      [],
    "elec_100_flat_results":    [],
    "elec_0_tou_results":  [(18, 22), (5, 9)],
    "elec_100_tou_results":  [(18, 22), (5, 9)],
    "elec_0_flat_cap_results":  [],
    "elec_100_flat_cap_results": [],
...
CASE_STYLE = {
    ...
    "elec_0_tou_results":        {"color": "#1f77b4", "linestyle": "-",  "label": "TOU 60%"},
    "elec_100_tou_results":      {"color": "#1f77b4", "linestyle": "--", "label": "TOU 100%"},
...
def _draw_peak_demand_panel(ax, all_results, baseline, x, xtick_rotation=0):
    """Render the System Peak Electrical Demand bars onto a given Axes."""
    peak_demands = [r["system_peak_gw"] for r in all_results]
    bar_colors   = [get_style(r["case_name"])[0] for r in all_results]
    bar_hatch    = ["" if get_style(r["case_name"])[1] == "-" else "//" for r in all_results]
    bar_labels   = [get_style(r["case_name"])[2] for r in all_results]

    bars = ax.bar(x, peak_demands, 0.7,
                  color=bar_colors, hatch=bar_hatch, edgecolor="white", alpha=0.85)
    ax.set_title("System Peak Electrical Demand", fontsize=16)
    ax.set_xticks(x)
    ax.set_xticklabels(bar_labels, fontsize=13,
                       rotation=xtick_rotation,
                       ha="right" if xtick_rotation else "center")
    ax.set_ylabel("Peak Electrical Demand (GW)", fontsize=14)
    ...
    for i, res in enumerate(all_results):
        if res["case_name"] != baseline["case_name"]:
            ...
            ax.text(bars[i].get_x() + bars[i].get_width() / 2,
                    ...
                    f"({pct:+.1f}%)", ha="center", va="top", fontsize=11, color="white",
                    fontweight="bold")


def _draw_capex_panel(ax, all_results, baseline, x, xtick_rotation=0):
    """Render the Total Capacity Expansion Cost bars onto a given Axes.

    Returns the list of GenX costs so the caller can warn on missing values.
    """
    ...
    bars = ax.bar(x, genx_costs, 0.7,
                  color=bar_colors, hatch=bar_hatch, edgecolor="white", alpha=0.85)
    ax.set_title("Total Capacity Expansion Cost", fontsize=16)
    ax.set_xticks(x)
    ax.set_xticklabels(bar_labels, fontsize=13,
                       rotation=xtick_rotation,
                       ha="right" if xtick_rotation else "center")
    ax.set_ylabel("System Cost ($B / year)", fontsize=14)
    ax.tick_params(axis="y", labelsize=12)

    for bar, val in zip(bars, genx_costs):
        if not np.isnan(val):
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height(),
                    f"${val:.2f}B", ha="center", va="bottom", fontsize=12)
...
def plot_comparison(all_results, baseline, output_dir):
    """Generate comparison plots: combined 2-panel figure + each panel saved
    standalone for use in slides/docs."""
    n_cases = len(all_results)
    x       = np.arange(n_cases)
    suptitle = f"Peak Shifting & Cost Comparison\nBaseline: {display_name(baseline['case_name'])}"
    ...
    _draw_peak_demand_panel(axes[0], all_results, baseline, x)
    genx_costs = _draw_capex_panel(axes[1], all_results, baseline, x)
    ...
    # Acronym legend shared by both standalone halves.
    # Drawn as invisible-line proxy artists so the legend frame stands on its own.
    acronym_handles = [
        Line2D([0], [0], color="none", label="TOU: Time-of-Use Rate"),
        Line2D([0], [0], color="none", label="CC: Capacity Charge"),
    ]

    # ── Standalone: System Peak Electrical Demand ──
    fig, ax = plt.subplots(figsize=(13, 7))
    fig.suptitle(suptitle, fontsize=18, fontweight="bold", y=0.97)
    _draw_peak_demand_panel(ax, all_results, baseline, x, xtick_rotation=45)
    ax.legend(handles=acronym_handles, loc="upper right",
              fontsize=12, frameon=True, handlelength=0, handletextpad=0,
              borderpad=0.6)
    fig.subplots_adjust(top=0.84, bottom=0.18, left=0.07, right=0.97)
    peak_path = os.path.join(output_dir, "peak_shifting_comparison_peak_demand.png")
    plt.savefig(peak_path, dpi=150, bbox_inches="tight", pad_inches=0.1)
    plt.close(fig)
    print(f"  Plot saved: {peak_path}")

    # ── Standalone: Total Capacity Expansion Cost ──
    fig, ax = plt.subplots(figsize=(13, 7))
    fig.suptitle(suptitle, fontsize=18, fontweight="bold", y=0.97)
    _draw_capex_panel(ax, all_results, baseline, x, xtick_rotation=45)
    ax.legend(handles=acronym_handles, loc="upper right",
              fontsize=12, frameon=True, handlelength=0, handletextpad=0,
              borderpad=0.6)
    fig.subplots_adjust(top=0.84, bottom=0.18, left=0.07, right=0.97)
    capex_path = os.path.join(output_dir, "peak_shifting_comparison_capex.png")
    plt.savefig(capex_path, dpi=150, bbox_inches="tight", pad_inches=0.1)
    plt.close(fig)
    print(f"  Plot saved: {capex_path}")
