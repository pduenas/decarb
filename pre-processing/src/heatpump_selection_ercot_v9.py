...
def load_neep_heat_pumps(neep_filepath: Path, verbose: bool = True) -> pd.DataFrame:
    ...
    Derived columns added to match the generic HP schema:
      ...
      pHVmx_  : heating capacity slope (1/°C), fractional loss per °C
      pACmx_  : cooling capacity loss magnitude (1/°C), fractional loss per °C, positive
      pHVeff_ : heating COP slope (per °C), negative (COP drops when cold)
      pACeff_ : cooling COP loss magnitude (per °C), positive (COP drops when hot)
      pHSPF   : HSPF (Region IV) from NEEP

    Capacity slopes are fractional (1/°C). COP slopes are absolute (COP/°C).
    ...
    for _, row in df.iterrows():
        ...
        # Heating capacity slope (1/°C) — fractional loss per °C
        h_caps_kw = np.array([
        ...
        )[0][0]) / pHVmx if pHVmx > 0 else 0.0

        # Cooling capacity loss magnitude (1/°C) — fractional loss per °C, positive
        c_caps_kw = np.array([
            row["Rated Capacity 47°F"],
        ...
        )[0][0]) / pACmx if pACmx > 0 else 0.0

        # Heating COP slope (per °C) — negative: COP drops as temp falls
        h_cops = np.array([
        ...
        # Cooling COP loss magnitude (per °C) — stored positive: COP drops as temp rises
        c_cops = np.array([
            pACeff,
...
# ---------------------------------------------------------------------------
# Capacity and COP at outdoor temperature
# ---------------------------------------------------------------------------
...
def capacity_at_temp(row: pd.Series, outdoor_temp_c: float, mode: str) -> float:
    ...
    using fractional slope (1/°C).

    For heating: capacity increases as outdoor temp rises toward design temp.
                 pHVmx_ is positive (from fit), delta is negative when cold,
                 so pHVmx * (1 + pHVmx_ * delta) shrinks capacity when cold.
    For cooling: capacity decreases as outdoor temp rises above design temp.
                 pACmx_ is stored as positive magnitude and explicitly subtracted.
...
def cop_at_temp(row: pd.Series, outdoor_temp_c: float, mode: str) -> float:
    ...
    using absolute slope (COP/°C).

    For heating: COP increases as outdoor temp rises toward design temp.
                 pHVeff_ is negative, so adding it * delta reduces COP when cold.
    For cooling: COP decreases as outdoor temp rises above design temp.
                 pACeff_ is stored as positive magnitude and explicitly subtracted.
