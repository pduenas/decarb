...
def print_statistics(stats, top_n=10):
    ...
    for id_, count in stats["top_n"]:
        print(f"    {id_:<20} {count:,}")
    print(f"\n  Bottom {top_n} IDs by count:")
    for id_, count in stats["bottom_n"]:
        print(f"    {id_:<20} {count:,}")
    print("  =================================")
...
def plot_bucketed_histogram(bucket_counts, out_dir):
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "bucketed_histogram.png", dpi=150)
    plt.close(fig)

def plot_cdf(aggregated_counts, out_dir):
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "cdf.png", dpi=150)
    plt.close(fig)



def plot_metadata_histograms(histograms, out_dir, max_cat_labels=30):
    for col, info in histograms.items():
        ...
        if info["type"] == "numeric":
            ...
            ax.bar(range(n), counts, edgecolor="black")
            ...
            ax.barh(labels[::-1], counts[::-1], edgecolor="black")
            ax.set_xlabel("Count")
            ax.set_title(col + title_suffix)

        fig.tight_layout()
        safe_col = col.replace(".", "_").replace("/", "_")
        fig.savefig(out_dir / f"{safe_col}.png", dpi=150)
        plt.close(fig)

# ── Analysis runners ──────────────────────────────────────────────────────────

def run_lookup_analysis(data, aggregated_counts, out_dir):
    """Bucketed histogram + CDF + top-N + substation plots from the lookup JSON."""
    print("\n── Lookup analysis ──────────────────────────────────────")
    stats = compute_statistics(aggregated_counts)
    print_statistics(stats)
    save_json(stats, out_dir / "lookup_stats.json")
    ...
    pd.DataFrame([csv_rows]).to_csv(out_dir / "lookup_stats.csv", index=False)
    print(f"  ✅ Saved: {out_dir / 'lookup_stats.csv'}")

    bucket_counts = bucketize_counts(aggregated_counts)
    save_json(bucket_counts, out_dir / "bucket_counts.json")

    plot_bucketed_histogram(bucket_counts, out_dir)
    plot_cdf(aggregated_counts, out_dir)

def run_metadata_analysis(df, out_dir):
    ...
    # Save JSON
    save_json(histograms, out_dir / "histogram_bins.json")

    # Save CSV — one row per (column, bin)
    csv_rows = []
    for col, info in histograms.items():
    ...
    pd.DataFrame(csv_rows).to_csv(out_dir / "histogram_bins.csv", index=False)
    print(f"  ✅ Saved: {out_dir / 'histogram_bins.csv'}")

    plot_metadata_histograms(histograms, out_dir)
...
def main():
    ...
    print(f"{'='*55}")
    run_metadata_analysis(df_all, ALL_OUT)

    # ── model_subset_stats: lookup analysis + metadata histograms for subset ──
    print(f"\n{'='*55}")
    print("MODEL SUBSET STATS")
    print(f"{'='*55}")
    run_lookup_analysis(data, aggregated_counts, SUBSET_OUT)
    run_metadata_analysis(df_subset, SUBSET_OUT)
