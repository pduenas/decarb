...
# Case naming — output path becomes in/decarb_inputs_alt/{CASE_NAME}/{bldg_id}/update_{UPDATE_NUM}/in/
UPDATE_NUM           = 0
...
def load_templates(parent_path: Path) -> dict:
    ...
    if RUN_CHP or RUN_SP:
        templates["chp"] = pd.read_csv(template_dir / "chp.csv")
    if RUN_TOPO:
        templates["topo"] = pd.read_csv(template_dir / "topo.csv")
    if RUN_HVAC or RUN_SP:
        templates["hvac"] = pd.read_csv(template_dir / "hvac.csv")
    if RUN_WH or RUN_SP:
        templates["wh"] = pd.read_csv(template_dir / "wh.csv")
    if RUN_SP:
        templates["sp"] = pd.read_csv(template_dir / "sp.csv")
...
def load_consumption(bldg_id: int, parent_path: Path, year: int) -> pd.DataFrame:
    ...
    if not isinstance(df.index, pd.DatetimeIndex):
        if len(df) == 35040:
            df.index = pd.date_range(start=datetime(year, 1, 1, 0, 0), periods=35040, freq="15min")
        elif len(df) == 8760:
            df.index = pd.date_range(start=datetime(year, 1, 1, 0, 0), periods=8760, freq="h")
        else:
            raise ValueError(f"Unexpected consumption data length for building {bldg_id}: {len(df)}")
    else:
...
def load_setpoints(bldg_id: int, parent_path: Path, year: int) -> pd.DataFrame:
    ...
    if not isinstance(df.index, pd.DatetimeIndex):
        if len(df) == 35040:
            ...
            raise ValueError(f"Unexpected setpoint data length for building {bldg_id}: {len(df)}")
    else:
        df.index = df.index - pd.Timedelta(minutes=15)
    return df
...
def generate_tm_data(consumption_df, setpoint_df, building_row):
    ...
    timestamps = pd.date_range(start=datetime(year, 1, 1, 0, 0), periods=n_steps, freq=freq)
    ...
    # Step 11: Equipment power — total minus all known end uses including lighting
    ...
    for out_col, src_col, scale in [
        ...
        if src_col in consumption_df.columns:
            df[out_col] = resample_if_needed(consumption_df[src_col], timestep, "mean").values / scale
        else:
            df[out_col] = 0.0
...
def _build_chp_df(template: pd.DataFrame, bldg_id: int,
    ...
    hp_heating_kw = float(row["heating_capacity_kw"].iloc[0])

    if building_row is not None and not is_heat_pump_building(building_row):
        rs_heating_kw, _ = get_resstock_capacities(building_row)
        heating_kw = (rs_heating_kw + hp_heating_kw) / 2
    else:
        heating_kw = hp_heating_kw
...
def _build_hvac_df(template: pd.DataFrame, bldg_id: int,
    ...
    if building_row is not None and not is_heat_pump_building(building_row):
        rs_heating_kw, rs_cooling_kw = get_resstock_capacities(building_row)
        heating_kw = (rs_heating_kw + hp_heating_kw) / 2
        cooling_kw = (rs_cooling_kw + hp_cooling_kw) / 2
    else:
        heating_kw = hp_heating_kw
        cooling_kw = hp_cooling_kw
    ...
    # Mitsubishi ASHP row always uses hp_summary (upgrade option)
...
def process_building(bldg_id: int) -> dict:
    ...
    try:
        ...
        output_dir = (parent_path / "in" / "decarb_inputs_alt" / CASE_NAME
...
if __name__ == "__main__":
    ...
    if RETRY_FAILED:
        log_path = parent_path / "in" / "decarb_inputs_alt" / CASE_NAME / "batch_failures.log"
