...
if __name__ == "__main__":
    ...
    overwrite_flag = False  # If False, skip buildings already in the summary file
    ...
    # Always start zero_hp_match fresh — it will be repopulated for any
    # buildings that still can't be matched on this run
    zero_hp_match = []

    for filepath in (consumption_files[:10] if debug else consumption_files):
        building_id = int(filepath.stem.split("-")[0])

        if building_id in already_processed:
            continue
