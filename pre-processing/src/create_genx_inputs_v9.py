...
COMMERCIAL_PROFILE_PATH: Path | None = Path(r"D:\shared\ercot_project\out\commercial_profiles\com_data_6716_bus_hourly.csv")
...
DISCOUNT_RATE     = 0.07   # 7% social discount rate for CRF calculation
MAX_CAP_MULTIPLIER = 2.0   # max new capacity = existing cap * this factor (e.g. 2.0 = allow doubling)
...
# NREL ATB 2024 overnight capital costs ($/kW) by unit type
# Source: NREL Annual Technology Baseline 2024, moderate scenario
NREL_OVERNIGHT_COST_PER_KW = {
    "ST":    3500,   # Coal steam turbine
    "GT":     750,   # Natural gas combustion turbine
    "CA":     900,   # Combined cycle (gas, aggregate)
    "CT":     900,   # Combined cycle (gas turbine portion)
    "IC":     800,   # Internal combustion
    "NUC":   6000,   # Nuclear
    # Fuel-type fallbacks
    "SUB":   3500,   # Subbituminous coal steam
    "BIT":   3500,   # Bituminous coal steam
    "NG":     900,   # Natural gas (default to CC)
    "WAT":   3000,   # Hydro (retrofit/upgrade cost)
    "PC":    3500,   # Petroleum coke steam
    "OTH":   1000,   # Other
    # VRE
    "WND":   1300,   # Onshore wind
    "SUN":   1050,   # Utility-scale solar PV
}

# Asset lifetimes (years) by unit type for CRF calculation
ASSET_LIFETIME = {
    "ST":   40,
    "GT":   30,
    "CA":   30,
...
def compute_crf(discount_rate: float, lifetime: int) -> float:
    """Capital Recovery Factor: annualizes overnight capital cost."""
    r, n = discount_rate, lifetime
    return r * (1 + r) ** n / ((1 + r) ** n - 1)


def _get_inv_cost(row: pd.Series, discount_rate: float = DISCOUNT_RATE) -> float:
    """
    Compute annualized investment cost ($/MW-yr) from NREL ATB 2024.
    Uses unit type first, then fuel type as fallback.

    Formula: Inv_Cost = Overnight_Cost ($/kW) * 1000 ($/MW) * CRF
    """
    unit_type = str(row.get("Unit Type Code", "")).strip().upper()
    fuel_code  = str(row.get("Fuel Type Code", "")).strip().upper()

    # Look up overnight cost and lifetime
    if unit_type in NREL_OVERNIGHT_COST_PER_KW:
        cost_per_kw = NREL_OVERNIGHT_COST_PER_KW[unit_type]
        lifetime    = ASSET_LIFETIME.get(unit_type, 30)
    elif fuel_code in NREL_OVERNIGHT_COST_PER_KW:
        cost_per_kw = NREL_OVERNIGHT_COST_PER_KW[fuel_code]
        lifetime    = ASSET_LIFETIME.get(fuel_code, 30)
    else:
        cost_per_kw = 1000   # generic fallback
        lifetime    = 30

    crf = compute_crf(discount_rate, lifetime)
    return round(cost_per_kw * 1000 * crf, 0)   # $/MW-yr
...
def build_thermal(
    ...
    for _, row in thermal_df.iterrows():
        ...
        records.append({
            ...
            "Can_Retire":             0,
            "Existing_Cap_MW":        cap,
            "Max_Cap_MW":             cap * MAX_CAP_MULTIPLIER,
            "Min_Cap_MW":             0,
            "Inv_Cost_per_MWyr":      _get_inv_cost(row),
...
def load_commercial_1bus(path: Path) -> np.ndarray:
    print(f"  Loading commercial profile from: {path.name}...")
    df = pd.read_csv(path, index_col=0)
    bus_cols = [c for c in df.columns if c.startswith("Load_MW_")]
    if not bus_cols:
        raise ValueError(f"No Load_MW_* columns found in {path.name}.")
    print(f"  Commercial buses found: {len(bus_cols)}")
    load_df = df[bus_cols].apply(pd.to_numeric, errors="coerce").fillna(0)
    total_load = load_df.sum(axis=1).values[:8760]
    print(f"  Commercial 1bus load: min={total_load.min():.1f} MW, "
          f"max={total_load.max():.1f} MW, mean={total_load.mean():.1f} MW, "
          f"total={total_load.sum()/1000:.1f} GWh/yr")
    return total_load
...
def build_demand(
    ...
    bus_map: dict = None,
    commercial_load: np.ndarray = None,
) -> pd.DataFrame:
    if network_mode == "7kbus":
        print("  NOTE: Demand profiles folder not compatible with 7kbus mode.")
    ...
    # 1bus: use pre-built demand profile
    if demand_profile_path is not None:
        ...
        if "Load_MW_z1" not in df.columns or "Time_Index" not in df.columns:
            raise ValueError(
                f"Demand profile {demand_profile_path.name} missing 'Time_Index' "
                f"and/or 'Load_MW_z1'. Got: {list(df.columns)}"
            )
        if commercial_load is not None:
            res_load = pd.to_numeric(df["Load_MW_z1"], errors="coerce").fillna(0).values
            combined = res_load + commercial_load[:len(res_load)]
            df["Load_MW_z1"] = combined
            print(f"  Residential: {res_load.sum()/1000:.1f} GWh/yr  "
                  f"Commercial: {commercial_load[:len(res_load)].sum()/1000:.1f} GWh/yr  "
                  f"Combined peak: {combined.max():.1f} MW")
        return df
...
def fetch_nsrdb_solar_cf(lat: float, lon: float, year: int,
    ...
    cell_temp = pvlib.temperature.sapm_cell(
        poa_global=poa["poa_global"], temp_air=df["Temperature"],
        wind_speed=df["Wind Speed"], a=-3.47, b=-0.0594, deltaT=3,
    )
    temp_coeff = 0.0045
...
def build_case(
    ...
    if BUILD_DEMAND:
        ...
        demand_df = build_demand(
            ...
            commercial_load=shared.get("commercial_load"),
        )
    ...
    if src_settings.exists():
        out_settings.mkdir(parents=True, exist_ok=True)
        src_files = list(src_settings.iterdir())
        copied = 0
        for src_file in src_files:
            dst_file = out_settings / src_file.name
            if src_file.is_file() and (not dst_file.exists() or OVERWRITE):
                shutil.copy2(src_file, dst_file)
                copied += 1
        print(f"  Copied: settings/ ({copied}/{len(src_files)} files)")
...
if __name__ == "__main__":
    ...
    print(f"    Discount rate: {DISCOUNT_RATE*100:.0f}%  |  New_Build: 1 (all generators)\n")
    ...
    if NETWORK_MODE == "1bus" and DEMAND_PROFILES_DIR is not None:
        if not DEMAND_PROFILES_DIR.exists():
            raise FileNotFoundError(f"DEMAND_PROFILES_DIR does not exist: {DEMAND_PROFILES_DIR}")
    ...
    # Load commercial profile once
    if NETWORK_MODE == "1bus" and COMMERCIAL_PROFILE_PATH is not None:
        if not COMMERCIAL_PROFILE_PATH.exists():
            raise FileNotFoundError(f"Commercial profile not found: {COMMERCIAL_PROFILE_PATH}")
        shared["commercial_load"] = load_commercial_1bus(COMMERCIAL_PROFILE_PATH)
    else:
        shared["commercial_load"] = None

    # -------------------------------------------------------------------------
    # Build outputs shared across all cases
    # -------------------------------------------------------------------------
    if BUILD_THERMAL:
        ...
        # Log a sample of investment costs for sanity check
        sample = thermal_df[["Resource", "cluster", "Inv_Cost_per_MWyr"]].head(5)
        print("  Sample investment costs ($/MW-yr):")
        for _, r in sample.iterrows():
            print(f"    {r['Resource'][:40]:40s} [{r['cluster']:3s}]  ${r['Inv_Cost_per_MWyr']:,.0f}")
    ...
    if BUILD_VARIABILITY:
        ...
        if input_cache_path.exists():
            ...
            nrel_profiles = fetch_nrel_profiles(
                ...
                api_key=NREL_API_KEY, email=NREL_EMAIL, year=NREL_YEAR,
            )
