...
PEAK_PERIODS = []
#     ("06:00", "10:00"),   # morning: 6am-10am (4 hrs)
...
def _is_peak_hour(hour):
    """Check if an hour falls within any of the PEAK_PERIODS."""
...
def _peak_period_spans():
    """Return list of (h0, h1) int tuples from PEAK_PERIODS."""
    spans = []
    for start_str, end_str in PEAK_PERIODS:
        h0 = int(start_str.split(":")[0])
        h1 = int(end_str.split(":")[0])
...
def plot_peak_histograms(df, label, output_path):
    ...
    # 4) Average hourly profile across all buildings (peak day)
    ax = axes[1, 1]
    profiles = df["_peak_day_profile"].dropna()
    valid_profiles = [p for p in profiles if isinstance(p, list) and len(p) == 24]
    if valid_profiles:
        avg_profile = np.mean(valid_profiles, axis=0)
        ax.plot(range(24), avg_profile, "o-", color="#333333", lw=2, markersize=4, zorder=3)
        # Shade peak period backgrounds
        for h0, h1 in spans:
        ...
        ax.set_title(f"Average Peak Day Profile\n({len(valid_profiles)} buildings)")
        ax.set_xlabel("Hour of Day")
        ...
        ax.text(0.5, 0.5, "Daily profiles not available", ha="center", va="center",
...
def plot_system_peak_day(df, label, output_path, case_dir, upgrade):
    """
    Load the aggregated timeseries, find the system peak day,
    and plot the 24-hour profile for that day.
    """
    spans = _peak_period_spans()

    # Try to load the aggregated timeseries CSV from the output directory
    agg_path = os.path.join(os.path.dirname(output_path),
                            os.path.basename(os.path.dirname(output_path)) + "_aggregated_ts.csv")

    # If aggregated CSV exists, use it directly
    if os.path.exists(agg_path):
        agg_df = pd.read_csv(agg_path)
        # Use first column (Grid Purchases)
        col = agg_df.columns[0]
        series = agg_df[col].values
    else:
        # Fall back: sum all building peak day profiles from the per-building results
        # This won't give exact system peak, so warn
        print(f"  WARNING: Aggregated CSV not found at {agg_path}")
        print(f"  Computing system profile by summing individual building timeseries...")

        # Load all building ts.csv files and sum them
        all_buy = None
        profiles = df["_peak_day_profile"].dropna()
        valid = [p for p in profiles if isinstance(p, list) and len(p) == 24]
        if not valid:
            print(f"  No valid profiles available for system peak day plot.")
            return
        # Can't reconstruct full year from peak day profiles alone
        # Use per-building full_peak_idx to identify the most common peak day
        peak_days = df["full_peak_idx"] // 24
        most_common_day = int(peak_days.mode().iloc[0])
        series = np.sum(valid, axis=0)  # fallback: average peak day
        col = "Grid Purchases (estimated)"

    if len(series) >= 8760:
        # Find the system peak hour and extract that day
        peak_hour_idx = int(np.argmax(series))
        day_start = (peak_hour_idx // 24) * 24
        day_end = day_start + 24
        day_profile = series[day_start:day_end]
        ...
        # If we only have 24 values (fallback), just plot them
        day_profile = series[:24]
        peak_hour = int(np.argmax(day_profile))
        peak_date_str = "estimated from building profiles"
...
def run_decarb_analysis(case_name, case_dir, output_dir):
    ...
    # System peak day plot
    sys_peak_path = os.path.join(output_dir, "system_peak_day_decarb.png")
    plot_system_peak_day(df, f"Decarb ({case_name})", sys_peak_path, case_dir, UPGRADE)
...
def run_resstock_analysis(case_name, case_dir, output_dir):
    ...
    # Load building IDs from building_mapping.csv (saved by decarb_output_analysis_v2.py)
    # Check output_dir first (where aggregation script saves it), then case_dir as fallback
    mapping_path = os.path.join(output_dir, "building_mapping.csv")
    if not os.path.exists(mapping_path):
        mapping_path = os.path.join(case_dir, "building_mapping.csv")
    if not os.path.exists(mapping_path):
        print(f"ERROR: building_mapping.csv not found in output_dir or case_dir")
        print(f"  Tried: {os.path.join(output_dir, 'building_mapping.csv')}")
        print(f"  Tried: {os.path.join(case_dir, 'building_mapping.csv')}")
        return
    ...
    plot_system_peak_day(df, f"ResStock Baseline ({case_name})", sys_peak_path, case_dir, UPGRADE)
