...
def create_hvac_file(output_dir: Path, template: pd.DataFrame,
                     bldg_id: int, hp_summary_df: pd.DataFrame) -> None:
    ...
    # Strip trailing zeros row before modifying (identified by ty == "0" or 0)
    zeros_mask = df["ty"].astype(str).isin(["0", "0.0"])
    zeros_row  = df[zeros_mask].copy()
    df         = df[~zeros_mask].copy()

    # Update existing rows (skip zeros)
    df.loc[df["pHVmx"] != 0, "pHVmx"] = heating_kw
    df.loc[df["pACmx"] != 0, "pACmx"] = cooling_kw
    # Append Mitsubishi ASHP row
    ...
    df = pd.concat([df, pd.DataFrame([mitsubishi_row]), zeros_row], ignore_index=True)
...
def create_chp_file(output_dir: Path, template: pd.DataFrame,
    ...
    # Strip trailing zeros row before modifying mx
    zeros_mask = df["ty"].astype(str).isin(["0", "0.0"])
    zeros_row  = df[zeros_mask].copy()
    df         = df[~zeros_mask].copy()
...
def process_building(bldg_id: int, parent_path: Path, metadata_df: pd.DataFrame,
    ...
    try:
        ...
        chp_df  = create_chp_file(output_dir, templates["chp"], bldg_id, hp_summary_df) if RUN_CHP  else templates["chp"].copy()
        hvac_df = create_hvac_file(output_dir, templates["hvac"], bldg_id, hp_summary_df) if RUN_HVAC else templates["hvac"].copy()
