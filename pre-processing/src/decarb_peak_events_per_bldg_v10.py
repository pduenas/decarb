...
import pandas as pd
import matplotlib
matplotlib.use("Agg")   # non-interactive backend; avoids Tk warnings in workers
...
# How to count buildings whose peak ties across multiple hours of day:
#   True  : each tied hour gets the full weight (totals inflate by tie count)
#   False : each tied hour gets weight / N (totals preserved)
TIE_INFLATE = False
...
def plot_peak_histograms(df, label, output_path, output_dir, case_name,
    ...
    if "summer_tied_hours" in df.columns:
        summer_counts = _hour_counts_tied(
            df["summer_tied_hours"], df["bldg_id"], weight_map,
            inflate=TIE_INFLATE,
        )
        tied_mode = True
    else:
    ...
    if "winter_tied_hours" in df.columns:
        winter_counts = _hour_counts_tied(
            df["winter_tied_hours"], df["bldg_id"], weight_map,
            inflate=TIE_INFLATE,
        )
    else:
    ...
    if tied_mode:
        note = (" (tied peaks counted in every hour)" if TIE_INFLATE
                else " (tied peaks split across hours)")
    else:
        note = ""
...
def plot_decarb_load_type_peaks(df, case_name, output_dir, weight_map=None):
    ...
    tie_note = ("tied peaks counted in every hour" if TIE_INFLATE
                else "tied peaks split across hours")
    fig.suptitle(
    ...
    for row_idx, metric in enumerate(metrics):
        for col_idx, (season, season_title) in enumerate(seasons):
            ...
            for hours, p, m in zip(tied_lists, peak_kwh, mean_kwh):
                if not hours:
                    continue
                share = 1.0 if TIE_INFLATE else 1.0 / len(hours)
                for h in hours:
                    if 0 <= h <= 23:
                        peak_by_hour[h] += p * share
                        mean_by_hour[h] += m * share
