...
# Single CSV with 8760 rows x 6716 bus columns (all in MW).
# For 1bus mode the bus columns are summed to get total commercial load,
# which is then added element-wise to the residential Load_MW_z1.
...
def load_commercial_1bus(path: Path) -> np.ndarray:
    """
    Load the commercial demand CSV, sum all bus columns to get a single
    8760-hour 1bus commercial load series (MW).
    Prints a progress/sanity check summary.
    """
    print(f"  Loading commercial profile from: {path.name}...")
    df = pd.read_csv(path, index_col=0)  # first col is 'time'

    # All columns should be bus load columns
    bus_cols = [c for c in df.columns if c.startswith("Load_MW_")]
    if not bus_cols:
        raise ValueError(f"No Load_MW_* columns found in {path.name}. Got: {list(df.columns[:5])}")

    print(f"  Commercial buses found: {len(bus_cols)}")

    # Convert to numeric and sum across all buses
    load_df = df[bus_cols].apply(pd.to_numeric, errors="coerce").fillna(0)
    total_load = load_df.sum(axis=1).values[:8760]

    # Sanity check prints
    print(f"  Commercial 1bus load (MW):")
    print(f"    Hours:   {len(total_load)}")
    print(f"    Min:     {total_load.min():.1f} MW")
    print(f"    Max:     {total_load.max():.1f} MW")
    print(f"    Mean:    {total_load.mean():.1f} MW")
    print(f"    Total:   {total_load.sum()/1000:.1f} GWh/yr")
...
def build_demand(
    ...
    If commercial_load (8760-length array, MW) is provided, it is added
    element-wise to Load_MW_z1 before writing.
    ...
    if demand_profile_path is not None:
        ...
        print(f"  Residential load rows: {n_load_rows} hours")

        if commercial_load is not None:
            res_load = pd.to_numeric(df["Load_MW_z1"], errors="coerce").fillna(0).values
            res_total = res_load.sum() / 1000
            com_total = commercial_load[:len(res_load)].sum() / 1000
            combined = res_load + commercial_load[:len(res_load)]
            df["Load_MW_z1"] = combined
            print(f"  Residential load:  {res_total:.1f} GWh/yr")
            print(f"  Commercial load:   {com_total:.1f} GWh/yr")
            print(f"  Combined load:     {combined.sum()/1000:.1f} GWh/yr  "
                  f"(peak: {combined.max():.1f} MW)")
...
if __name__ == "__main__":
    ...
    # Load commercial profile once (shared across all cases)
