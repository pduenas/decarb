...
TARIFF_LABEL         = "flat_ihg"    # short label for the tariff configuration
...
RUN_BDG_II           = True
RUN_IN               = True   # in.csv
RUN_OTHER            = True   # abs.csv, pv.csv, bess.csv, wind.csv, ev.csv
RUN_CHP              = True   # chp.csv
RUN_TOPO             = True   # topo.csv
...
def get_building_row(bldg_id: int, metadata_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if bldg_id in metadata_df.index:
        row = metadata_df.loc[[bldg_id]]
    elif "bldg_id" in metadata_df.columns:
...
def get_resstock_capacities(building_row):
    """Get heating/cooling capacities from ResStock metadata (BTU/hr → kW)."""
    heating_btu = float(building_row["out.capacity.heating..btu_per_hr"].iloc[0])
    cooling_btu = float(building_row["out.capacity.cooling..btu_per_hr"].iloc[0])
    return heating_btu * BTU_HR_TO_KW, cooling_btu * BTU_HR_TO_KW


def is_heat_pump_building(building_row):
    """Check if the building's heating system is a heat pump."""
    heating_eff = str(building_row["in.hvac_heating_efficiency"].iloc[0])
    return any(kw in heating_eff for kw in ("ASHP", "MSHP", "Heat Pump"))
...
def generate_tm_data(consumption_df, setpoint_df, building_row):
    ...
    # Step 6: Heating energy (kept for total energy calculation)
    heating_cols = [
    ...
    # Step 7: Cooling energy (kept for total energy calculation)
    cooling_cols = [
    ...
    # Step 10: Total energy (kept as fallback for equipment)
    total_cols = [
    ...
    # Step 11: Equipment power (pQequip) — total minus all known end uses
    # Subtracts heating, cooling, EV, hot water, AND lighting to avoid
    # double-counting (since pQlight is added separately in the model)
    lighting_kwh = resample_if_needed(lighting_15min, timestep, "sum").values
    equip_kwh = total_kwh.copy()
    for part in (heating_kwh, cooling_kwh, ev_kwh, hw_kwh, lighting_kwh):
        if part is not None:
            equip_kwh -= part
    # # Alternative: plug loads only (uncomment to use)
    # plug_col = "out.electricity.plug_loads.energy_consumption..kwh"
    # if plug_col in consumption_df.columns:
    #     equip_kwh = resample_if_needed(consumption_df[plug_col], timestep, "sum").values
...
def create_sp_file(output_dir: Path, template: pd.DataFrame,
    ...
    bldg_id = building_row.index[0] if building_row.index.name in ("bldg_id", "building_id") else building_row.iloc[0].get("bldg_id", building_row.iloc[0].get("building_id", "?"))
    df = template.copy()
    ...
    if "Shared Heating" in heating_id:
        ...
        hvac_heat_only = hvac_df[
            (hvac_df["ty"].notna()) &
            (hvac_df["ty"].astype(str).str.lower() != "none") &
            (hvac_df["pHVmx"] > 0) &
            (hvac_df["pACmx"] == 0) &
            (hvac_df["ty"].astype(str).str.startswith(fuel))
        ]["ty"].tolist()
        chp_heat_only = chp_df[
            (chp_df["ty"].notna()) &
            (chp_df["ty"].astype(str).str.lower() != "none") &
            (chp_df["ty"].astype(str).str.startswith(fuel))
        ]["ty"].tolist()
        pool = hvac_heat_only + chp_heat_only
        if not pool:
            pool = hvac_df[
                (hvac_df["ty"].notna()) &
                (hvac_df["pHVmx"] > 0) & (hvac_df["pACmx"] == 0)
            ]["ty"].tolist() + chp_df[
                (chp_df["ty"].notna()) &
                (chp_df["ty"].astype(str).str.lower() != "none")
            ]["ty"].tolist()
        chosen = rng.choice(pool)
        print(f"  WARNING bldg {bldg_id}: heating '{heating_id}' → shared heating '{chosen}'")
        heating_id = chosen
    ...
    if cooling_shared:
        ...
        if not pool:
            raise ValueError(f"No cooling-only HVAC options for building {bldg_id}")
        chosen = rng.choice(pool)
        print(f"  WARNING bldg {bldg_id}: cooling '{cooling_id}' → cooling-only '{chosen}'")
        cooling_id = chosen
...
def create_wh_file(output_dir: Path, template: pd.DataFrame,
                   building_row: pd.DataFrame = None) -> None:
    """Create wh.csv with per-building tank size from ResStock data."""
    df = template.copy()

    if building_row is not None:
        # Convert tank size: gallons → kWh thermal storage (ΔT = 45°C)
        # kWh = gallons × 3.78541 L/gal × 4.186 kJ/(kg·°C) × 45°C / 3600 kJ/kWh
        GAL_TO_KWH = 3.78541 * 4.186 * 45 / 3600  # ≈ 0.198
        tank_gal = float(building_row["out.params.size_water_heater..gal"].iloc[0])
        tank_kwh = round(tank_gal * GAL_TO_KWH, 2)

        # Strip trailing zeros row
        zeros_mask = df["ty"].astype(str).isin(["0", "0.0"])
        zeros_row = df[zeros_mask].copy()
        df = df[~zeros_mask].copy()

        # Set tank size for all non-tankless units (tankless have tank=0)
        tankless_mask = df["tank"].astype(float) == 0
        df.loc[~tankless_mask, "tank"] = round(tank_kwh)

        df = pd.concat([df, zeros_row], ignore_index=True)

    df.to_csv(output_dir / "wh.csv", index=False)


def _build_chp_df(template: pd.DataFrame, bldg_id: int,
                  hp_summary_df: pd.DataFrame,
                  building_row: pd.DataFrame = None) -> pd.DataFrame:
    """Build per-building chp DataFrame without saving to disk.
    Uses ResStock capacity for non-heat-pump buildings, hp_summary for heat pumps."""
    df = template.copy()
    if building_row is not None and not is_heat_pump_building(building_row):
        heating_kw, _ = get_resstock_capacities(building_row)
    else:
        if bldg_id not in hp_summary_df.index:
            raise ValueError(f"Building {bldg_id} not found in heat pump summary file")
        row = hp_summary_df.loc[[bldg_id]]
        heating_kw = float(row["heating_capacity_kw"].iloc[0])
    zeros_mask = df["ty"].astype(str).isin(["0", "0.0"])
    zeros_row  = df[zeros_mask].copy()
    df         = df[~zeros_mask].copy()
    df["mx"] = heating_kw
...
def _build_hvac_df(template: pd.DataFrame, bldg_id: int,
    ...
    # Use ResStock capacities for non-heat-pump buildings
    if building_row is not None and not is_heat_pump_building(building_row):
        heating_kw, cooling_kw = get_resstock_capacities(building_row)
    else:
    ...
    # Always add Mitsubishi HP option from hp_summary (this is the upgrade option)
    hp_index = str(row["hp_index"].iloc[0])
    hp_heating_kw = float(row["heating_capacity_kw"].iloc[0]) * count
    hp_cooling_kw = float(row["cooling_capacity_kw"].iloc[0]) * count
    mitsubishi_row = {
...
def create_hvac_file(output_dir: Path, template: pd.DataFrame,
                     bldg_id: int, hp_summary_df: pd.DataFrame,
                     building_row: pd.DataFrame = None) -> pd.DataFrame:
    """Create hvac.csv from template, updating capacities and adding Mitsubishi row."""
    df = _build_hvac_df(template, bldg_id, hp_summary_df, building_row)
    df.to_csv(output_dir / "hvac.csv", index=False)
    return df


def create_topo_file(output_dir: Path, template: pd.DataFrame,
                     sp_df: pd.DataFrame = None) -> None:
    """Create topo.csv ensuring the building's installed CHP unit is connected."""
    df = template.copy()

    if sp_df is not None:
        # Get all installed CHP units (z0=1, not "0")
        installed = sp_df[
            (sp_df["pCHPz0"].astype(str) == "1") &
            (sp_df["pCHP0"].astype(str) != "0")
        ]["pCHP0"].tolist()

        # Strip trailing zeros row
        zeros_mask = df["up"].astype(str).isin(["0", "0.0"])
        zeros_row = df[zeros_mask].copy()
        df = df[~zeros_mask].copy()

        # Add any installed CHP not already in topo
        existing = df["up"].tolist()
        for chp_name in installed:
            if chp_name not in existing:
                new_row = {
                    "up": chp_name,
                    "lo": "Building",
                    "in": "hot air",
    ...
    df.to_csv(output_dir / "topo.csv", index=False)


def create_chp_file(output_dir: Path, template: pd.DataFrame,
                    bldg_id: int, hp_summary_df: pd.DataFrame,
                    building_row: pd.DataFrame = None) -> pd.DataFrame:
    """Create chp.csv from template with mx column set to building heating capacity."""
    df = _build_chp_df(template, bldg_id, hp_summary_df, building_row)
...
def _worker_init(parent_path: Path, metadata_df, regression_coeffs: dict,
                 templates: dict, elevation_cache: dict,
                 hp_summary_df) -> None:
    """Initialize shared state once per worker process."""
    _worker_state["parent_path"]       = parent_path
    _worker_state["regression_coeffs"] = regression_coeffs
    _worker_state["templates"]         = templates
    _worker_state["elevation_cache"]   = elevation_cache
    # Index by bldg_id for O(1) lookup
    id_col = "bldg_id" if "bldg_id" in metadata_df.columns else "building_id"
    _worker_state["metadata_df"]   = metadata_df.set_index(id_col)
    _worker_state["hp_summary_df"] = hp_summary_df.set_index("bldg_id")
...
def process_building(bldg_id: int) -> dict:
    """
    ...
    parent_path       = _worker_state["parent_path"]
    metadata_df       = _worker_state["metadata_df"]
    regression_coeffs = _worker_state["regression_coeffs"]
    templates         = _worker_state["templates"]
    elevation_cache   = _worker_state["elevation_cache"]
    hp_summary_df     = _worker_state["hp_summary_df"]

    try:
        # --- Load per-building data ---
        building_row = get_building_row(bldg_id, metadata_df)

        needs_consumption = RUN_TM or RUN_IN or RUN_OTHER or RUN_BDG_II
        needs_setpoints   = RUN_TM

        consumption_df = load_consumption(bldg_id, parent_path, YEAR) if needs_consumption else None
        setpoint_df    = load_setpoints(bldg_id, parent_path, YEAR)   if needs_setpoints   else None
        ...
        # Build chp/hvac dfs — now passing building_row for correct capacity sizing
        chp_df  = None
        hvac_df = None
        if RUN_CHP or RUN_SP:
            chp_df = (create_chp_file(output_dir, templates["chp"], bldg_id, hp_summary_df, building_row)
                      if RUN_CHP
                      else _build_chp_df(templates["chp"], bldg_id, hp_summary_df, building_row))
        if RUN_HVAC or RUN_SP:
            hvac_df = (create_hvac_file(output_dir, templates["hvac"], bldg_id, hp_summary_df, building_row)
                       if RUN_HVAC
                       else _build_hvac_df(templates["hvac"], bldg_id, hp_summary_df, building_row))

        if RUN_WH:
            create_wh_file(output_dir, templates["wh"], building_row)
        ...
        # Topo depends on sp to know which CHP unit is installed
        if RUN_TOPO:
            create_topo_file(output_dir, templates["topo"], sp_df)

        # bdg_i depends on sp to set pBhvac correctly
        if RUN_BDG_I:
            if sp_df is not None:
                pBhvac = max(int((sp_df["pHVACz0"] == 1).sum()), 1)
                create_bdg_i_file(output_dir, templates["bdg_i"],
                                  building_row, elevation_cache, pBhvac=pBhvac)
            else:
                create_bdg_i_file(output_dir, templates["bdg_i"],
                                  building_row, elevation_cache)
...
if __name__ == "__main__":
    ...
    coeff_file = parent_path / "out" / "thermal_model" / "baseline_internal_gains" / "regression_coeff_baseline_internal_gains.json"
    ...
    with ProcessPoolExecutor(
        ...
        initargs=(parent_path, metadata_df, regression_coeffs,
                  templates, elevation_cache, hp_summary_df)
    ) as executor:
        futures = {
            executor.submit(process_building, bldg_id): bldg_id
        ...
        for future in as_completed(futures):
            ...
            if not result["success"]:
                print(f"  ERROR bldg {result['bldg_id']}: {result['error'].splitlines()[-1]}")
            if len(results) % 100 == 0 or len(results) == len(bldg_ids):
                elapsed = time.perf_counter() - wall_start
                rate    = len(results) / elapsed if elapsed > 0 else 0
                print(f"[{len(results)}/{len(bldg_ids)}] {elapsed:.0f}s elapsed  "
                      f"({rate:.1f} bldg/s)")
