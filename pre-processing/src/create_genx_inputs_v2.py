...
def build_demand(hourly_df: pd.DataFrame, network_mode: str) -> pd.DataFrame:
    ...
    Includes required Rep_Periods, Timesteps_per_Rep_Period, Sub_Weights columns
    for running without Time Domain Reduction (full 8760-hour year).
    Load values come from OPF Total MW Load column.
    """
...
if __name__ == "__main__":
    ...
    if demand_df is not None:
        write_csv(demand_df, paths["out_demand"], index=True)
