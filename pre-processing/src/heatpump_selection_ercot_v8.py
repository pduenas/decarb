...
def load_neep_heat_pumps(neep_filepath: Path, verbose: bool = True) -> pd.DataFrame:
    ...
    Derived columns added to match the generic HP schema:
      ...
      pHVmx_  : heating capacity slope (kW/°C), negative (capacity drops when cold)
      pACmx_  : cooling capacity loss magnitude (kW/°C), positive (capacity drops when hot)
    ...
    All slopes are absolute (kW/°C or COP/°C), not fractional.
    ...
    for _, row in df.iterrows():
        ...
        # Heating capacity slope (kW/°C) — negative: capacity drops as temp falls
        h_caps_kw = np.array([
        ...
        # Cooling capacity loss magnitude (kW/°C) — stored positive: capacity drops as temp rises
        c_caps_kw = np.array([
            row["Rated Capacity 47°F"],
...
def capacity_at_temp(row: pd.Series, outdoor_temp_c: float, mode: str) -> float:
    ...
    using absolute slope (kW/°C).

    For heating: capacity increases as outdoor temp rises toward design temp.
                 pHVmx_ is negative, so adding it * delta reduces capacity when cold.
