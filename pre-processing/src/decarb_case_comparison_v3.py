...
def plot_comparison(all_results, baseline, output_dir):
    ...
    # ── 1) Average hourly electrical demand profiles ──
    ax = axes[0, 0]
    for res in all_results:
        color, ls, label = get_style(res["case_name"])
        grid        = res["_grid_gw"]
        ...
        ax.plot(range(24), avg_by_hour, linestyle=ls, color=color, lw=lw, label=label)
    ...
    ax.set_title("Average Hourly Electrical Demand (All Year)")
    ...
    # Use each case's tariff color for the bar face; hatch dashed-cases for distinction
    ...
    bars1 = ax.bar(x - width / 2, peak_vals,    width,
                   color=bar_colors, hatch=bar_hatch,
                   label="Peak Electrical Demand (GWh)", edgecolor="white", alpha=0.85)
    bars2 = ax.bar(x + width / 2, offpeak_vals, width,
                   color=bar_colors, hatch=bar_hatch,
                   label="Off-Peak Electrical Demand (GWh)", edgecolor="white", alpha=0.45)
    ax.set_title("Electrical Demand by Peak / Off-Peak Period")
    ax.set_xticks(x)
    ax.set_xticklabels(bar_labels, fontsize=9)
    ax.set_ylabel("Electrical Demand (GWh)")
    ax.legend(fontsize=8)
    for bar in bars1:
...
def plot_hourly_delta(all_results, baseline, output_dir):
    ...
    fig.suptitle(f"Hourly Electrical Demand Change vs Baseline ({baseline['case_name']})",
    ...
    for i, res in enumerate(non_baseline):
        ...
        # Use the case's tariff color; positive bars slightly darker, negative slightly lighter
        bar_colors = [color for _ in delta]
        bar_alpha  = [0.85 if d > 0 else 0.45 for d in delta]

        for h, (d, c, a) in enumerate(zip(delta, bar_colors, bar_alpha)):
            ax.bar(h, d, color=c, alpha=a, edgecolor="white", lw=0.5)
...
def plot_avg_and_peak_day(all_results, output_dir):
    ...
    Legend labels use the clean case descriptions from CASE_STYLE.
    """
    ...
    for res in all_results:
        ...
        axes[0].plot(range(24), avg_by_hour, color=color, linestyle=ls, lw=2, label=full_label)
        ...
        axes[1].plot(day_hours, day_grid, color=color, linestyle=ls, lw=2, label=full_label)
