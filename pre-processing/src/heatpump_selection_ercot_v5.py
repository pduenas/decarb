...
def load_neep_heat_pumps(neep_filepath: Path, verbose: bool = True) -> pd.DataFrame:
    ...
    Derived columns added to match the generic HP schema:
      ...
      pACmx   : rated cooling capacity at 95°F in kW
      pHVeff  : COP at rated heating capacity at 47°F
      pACeff  : COP at rated cooling capacity at 95°F
      temp    : reference temperature in °C (47°F = 8.33°C)
      pHVmx_  : heating capacity slope (kW/°C), fitted through 5°F/17°F/47°F
      pACmx_  : cooling capacity slope (kW/°C), fitted through 47°F/82°F/95°F
    ...
    for _, row in df.iterrows():
        ...
        pACmx  = row["Rated Capacity 95°F"] * BTU_TO_KW
        pHVeff = row["COP at Rated Capacity 47°F"]
        pACeff = row["COP at Rated Capacity 95°F"]

        # Heating capacity slope
        h_caps_kw = np.array([
            row["Rated Capacity 5°F"],
            row["Rated Capacity 17°F"],
            row["Rated Capacity 47°F"],
        ]) * BTU_TO_KW
        pHVmx_ = float(np.linalg.lstsq(
            h_delta_c.reshape(-1, 1), h_caps_kw - pHVmx, rcond=None
        )[0][0])

        # Cooling capacity slope
        c_caps_kw = np.array([
            row["Rated Capacity 47°F"],
            row["Rated Capacity 82°F"],
            row["Rated Capacity 95°F"],
        ]) * BTU_TO_KW
        pACmx_ = float(np.linalg.lstsq(
            c_delta_c.reshape(-1, 1), c_caps_kw - pHVmx, rcond=None
        )[0][0])
        ...
        pACeff_ = float(np.linalg.lstsq(
            c_delta_c.reshape(-1, 1), c_cops - pHVeff, rcond=None
        )[0][0])
...
def capacity_at_temp(row: pd.Series, outdoor_temp_c: float, mode: str) -> float:
    ...
    by applying the linear loss rate from the reference temperature.
    ...
    if mode == "heating":
        cap = row["pHVmx"] + row["pHVmx_"] * delta
    else:
        cap = row["pACmx"] + row["pACmx_"] * delta
    return max(cap, 0.0)
...
def cop_at_temp(row: pd.Series, outdoor_temp_c: float, mode: str) -> float:
    ...
    delta = outdoor_temp_c - row["temp"]
    if mode == "heating":
        cop = row["pHVeff"] + row["pHVeff_"] * delta
    else:
        cop = row["pACeff"] + row["pACeff_"] * delta
    return max(cop, 1.0)
...
def find_multi_unit_options(
    ...
    N identical units of the same type.

    For each N in [2, 3, 4]:
      - Each unit must meet peak_load_btu_hr / N (heating)
      - Each unit must meet peak_cooling_btu_hr / N (cooling)
...
def _make_hp_heating_subplot(
    ...
    heating sizing temperature, y-axis is total installed cost.
    """
    cop_vals = hp_df["pHVeff"]
    scatter = ax.scatter(
    ...
    for _, row in hp_df.iterrows():
        ax.annotate(row["ty"], (row["capacity_kw_heating"], row["cost_total"]),
                    fontsize=7, ha="left", va="bottom")
    plt.colorbar(scatter, ax=ax).set_label("Heating COP at reference temp")
...
def _make_hp_cooling_subplot(
    ...
    cooling sizing temperature, y-axis is total installed cost.
    """
    cop_vals = hp_df["pACeff"]
    scatter = ax.scatter(
    ...
    for _, row in hp_df.iterrows():
        ax.annotate(row["ty"], (row["capacity_kw_cooling"], row["cost_total"]),
                    fontsize=7, ha="left", va="bottom")
    plt.colorbar(scatter, ax=ax).set_label("Cooling COP at reference temp")
...
def _save_hp_figure(
    ...
    _make_hp_heating_subplot(ax_h, hp_df, heating_req, outdoor_temp_f_heating)
    _make_hp_cooling_subplot(ax_c, hp_df, cooling_req, outdoor_temp_f_cooling)
...
if __name__ == "__main__":
    ...
    plot_flag = True
    save_parquet = False
    debug = True  # If True, only process the first 10 buildings
    ...
    n_hp_total = len(hp_base)

    heatpump_dir = BASE_DIR / "out" / "heatpump_files"
    heatpump_dir.mkdir(parents=True, exist_ok=True)

    hp_base.to_csv(heatpump_dir / "available_heatpumps.csv", index=False)
    ...
    for filepath in (consumption_files[:10] if debug else consumption_files):
        ...
        try:
            ...
            print(f"{n_sufficient} of {n_hp_total} HP unit(s) meet capacity requirements for building {building_id}")
            ...
            if n_sufficient == 0:
                ...
                if hp_df_multi is None:
                    ...
                    print(f"Building {building_id}: no matching HP unit even with up to 4 units.")
                else:
