...
def create_building_timeseries(
    ...
    for _, row in merged_bldg_df.iterrows():
        ...
        try:
            ...
            # Outdoor temp is required for temp_diff_lag — skip building if missing
            ...
            df_bldg.rename(columns={
                OUTDOOR_TEMP_COL:                  "outdoor_temp_c",    # <-- added
...
def compute_regression_coefficients(
    ...
    for bldg_id, df in all_ts_dict.items():
        ...
        # Cast to float64 — lsq_linear and newey_west_se require double precision
        ...
        # -------------------------------------------
        # Joint HAC F-test: H0: k1 = k2 = k3 = 0
        # R is identity (test all coefficients jointly)
        # F = (1/q) * coef' * (R * V_hac * R')^{-1} * coef
        # where q = number of restrictions = n_features
        # -------------------------------------------
        q = n_features
        try:
        ...
        # p-value from chi-squared(q) distribution (Wald test, large T)
