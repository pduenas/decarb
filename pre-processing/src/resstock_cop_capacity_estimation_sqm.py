...
SQFT_TO_M2 = 0.092903  # 1 ft² = 0.092903 m²
...
def process_building(
    ...
    Capacity is normalized to kW/m².
    ...
    try:
        ...
        # Capacity: normalize to kW/m², then bin, find peak, fit decline
        h_cap_norm = data["heating_capacity_kw"] / area_m2
        h_cap_bins = bin_capacity_by_delta_t(h_cap_norm, data["heating_cap_delta_t"])
        h_rated, h_cap_loss, h_peak_dt, h_cap_r2 = fit_capacity_from_bins(h_cap_bins)

        c_cap_norm = data["cooling_capacity_kw"] / area_m2
        c_cap_bins = bin_capacity_by_delta_t(c_cap_norm, data["cooling_cap_delta_t"])
        ...
        return {
            ...
            "heating_rated_cap_kw_per_m2": h_rated,
            "heating_cap_loss_kw_per_m2_per_c": h_cap_loss,
            "heating_cap_peak_delta_t": h_peak_dt,
            "heating_cap_r2": h_cap_r2,
            "heating_cap_n_obs": len(data["heating_capacity_kw"]),
            "cooling_rated_cap_kw_per_m2": c_rated,
            "cooling_cap_loss_kw_per_m2_per_c": c_cap_loss,
...
def aggregate_capacity_results(all_results: list[dict]) -> dict:
    ...
    for mode, records in [("heating", heating_records), ("cooling", cooling_records)]:
        ...
        # Fit capacity decline from the peak of the aggregate curve
        if len(agg) >= 3:
            # Build a pseudo cap_bins DataFrame for fit_capacity_from_bins
...
def main():
    ...
    output_dir = BASE_DIR / "out" / "cop_capacity_analysis_sqm"
    ...
    # --- Build area lookup (sqft → m²) ---
    area_m2_map = {}
    for bid in available_ids:
        if bid in metadata_df.index:
            sqft = float(metadata_df.loc[bid, "in.sqft..ft2"])
            area_m2_map[bid] = sqft * SQFT_TO_M2
        else:
            area_m2_map[bid] = 1.0  # fallback
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=n_workers) as executor:
        futures = {
            executor.submit(
                ...
                bid in electric_heating_ids, area_m2_map[bid],
            ): bid
            for bid in available_ids
        }
        done_count = 0
    ...
    for r in all_results:
        summary_rows.append({
            ...
            "heating_rated_cap_kw_per_m2": r["heating_rated_cap_kw_per_m2"],
            "heating_cap_loss_kw_per_m2_per_c": r["heating_cap_loss_kw_per_m2_per_c"],
            "heating_cap_peak_delta_t": r["heating_cap_peak_delta_t"],
            "heating_cap_r2": r["heating_cap_r2"],
            "heating_cap_n_obs": r["heating_cap_n_obs"],
            "cooling_rated_cap_kw_per_m2": r["cooling_rated_cap_kw_per_m2"],
            "cooling_cap_loss_kw_per_m2_per_c": r["cooling_cap_loss_kw_per_m2_per_c"],
    ...
    for mode in ["heating", "cooling"]:
        ...
        if not agg_df.empty:
            ...
            print(f"{mode.capitalize()} capacity aggregate: rated = {rated:.4f} kW/m², "
                  f"loss = {loss:.6f} kW/m²/°C, peak at delta_T = {peak_dt:.1f}°C  (R² = {r2:.3f})")
...
def plot_capacity_spread(output_dir: Path, metadata_df: pd.DataFrame = None):
    ...
    for mode, type_col, n_obs_col, per_bldg_file in [
        ...
        for htype in sorted(df_mode[type_col].dropna().unique()):
            ...
            category_fit_rows.append({
                "mode": mode,
                "hvac_type": htype,
                "rated_cap_kw_per_m2": rated,
                "cap_loss_kw_per_m2_per_c": loss,
    ...
    mode_configs = [
        {"mode": "heating", "type_col": "heating_category",
         "slope_col": "heating_cap_loss_kw_per_m2_per_c", "intercept_col": "heating_rated_cap_kw_per_m2",
         "n_obs_col": "heating_cap_n_obs",
         "per_bldg_file": "heating_capacity_by_delta_t_per_building.parquet"},
        {"mode": "cooling", "type_col": "cooling_category",
         "slope_col": "cooling_cap_loss_kw_per_m2_per_c", "intercept_col": "cooling_rated_cap_kw_per_m2",
    ...
    for cfg in mode_configs:
        ...
        if per_bldg_path.exists():
            ...
            for i, htype in enumerate(hvac_types):
                ...
                ax.fill_between(binned["delta_t_bin_center"],
                                ...
                                alpha=0.15, color=colors[i])
            ax.set_xlabel("Delta T (°C)")
            ax.set_ylabel("Capacity (kW/m²)")
            ax.set_title(f"{mode.capitalize()} Capacity (95th pct, kW/m²) vs Delta-T by HVAC Type")
            ax.legend(fontsize=8, loc="best")
            ax.grid(True, alpha=0.3)
            fig.tight_layout()
        ...
        if valid:
            ...
            bp1 = ax1.boxplot(v_slopes, tick_labels=labels, patch_artist=True, showfliers=False)
            for patch, c in zip(bp1["boxes"], colors): patch.set_facecolor(c); patch.set_alpha(0.5)
            ax1.set_ylabel("Capacity Loss (kW/m²/°C)"); ax1.set_title(f"{mode.capitalize()} Capacity Loss Slope")
            ax1.tick_params(axis="x", rotation=30, labelsize=8); ax1.grid(True, alpha=0.3, axis="y")
            bp2 = ax2.boxplot(v_intercepts, tick_labels=labels, patch_artist=True, showfliers=False)
            for patch, c in zip(bp2["boxes"], colors): patch.set_facecolor(c); patch.set_alpha(0.5)
            ax2.set_ylabel("Rated Capacity (kW/m²)"); ax2.set_title(f"{mode.capitalize()} Rated Capacity")
            ax2.tick_params(axis="x", rotation=30, labelsize=8); ax2.grid(True, alpha=0.3, axis="y")
        fig.tight_layout()
        ...
        for i, htype in enumerate(hvac_types):
            ...
            ax.scatter(sub[cfg["slope_col"]], sub[cfg["intercept_col"]],
                       label=f"{htype} (n={len(sub)})",
                       color=colors[i], alpha=0.4, s=15, edgecolors="none")
        ax.set_xlabel("Capacity Loss (kW/m²/°C)"); ax.set_ylabel("Rated Capacity (kW/m²)")
