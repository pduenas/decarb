...
def compute_seasonal_peak(series, n_hours):
    """Return summer and winter peak hour-of-day, peak value, and seasonal mean.

    Uses the same summer/winter slices as compute_peaks_from_hourly so all
    bucketing in the script uses identical season definitions.
    """
    blank = {
...
def plot_decarb_load_type_peaks(df, case_name, output_dir, weight_map=None):
    ...
    Cols : summer peak hour (Jun-Aug) / winter peak hour (Jan-Mar, Oct-Dec)

    Every building appears in both columns. For each column, buildings are
    bucketed by the hour-of-day of their seasonal peak for that load type.

    Each cell shows side-by-side grouped bars per hour-of-day, in GWh:
      - Solid bar  : sum of (weight x seasonal peak consumption value)
      - Hatched bar: sum of (weight x seasonal MEAN consumption)
        (summer column uses summer mean; winter column uses winter mean,
         so peak and avg describe the same season and are like-for-like.)
    """
    weight_map = weight_map or {}
    is_tou = _is_tou_case(case_name)
    spans = _peak_period_spans()
    ...
    for row_idx, metric in enumerate(metrics):
        for col_idx, (season, season_title) in enumerate(seasons):
            ...
            peak_hour_col = f"{metric}_{season}_peak_hour"
            peak_val_col = f"{metric}_{season}_peak_val"
            mean_col = f"{metric}_{season}_mean"

            if peak_hour_col not in df.columns or mean_col not in df.columns:
                ax.text(0.5, 0.5,
                        f"{peak_hour_col} / {mean_col} missing",
                        ha="center", va="center", transform=ax.transAxes)
                continue

            sub = df[["bldg_id", peak_hour_col, peak_val_col, mean_col]].dropna(
                subset=[peak_hour_col]
            )
            ...
            peak_gwh = (sub[peak_val_col].astype(float).values
                        * weights.values / UNIT_SCALE)
            mean_gwh = (sub[mean_col].astype(float).values
                        * weights.values / UNIT_SCALE)
            hours = sub[peak_hour_col].astype(int).values

            peak_by_hour = np.zeros(24)
            mean_by_hour = np.zeros(24)
            for h, p, m in zip(hours, peak_gwh, mean_gwh):
                peak_by_hour[h] += p
                mean_by_hour[h] += m
...
def run_decarb_analysis(case_name, case_dir, output_dir):
    ...
    # 3x2 seasonal peak-vs-avg by load type (heating / cooling / electricity)
    needed = []
    for m in ["heat", "cool", "elec"]:
        needed += [f"{m}_summer_peak_hour", f"{m}_winter_peak_hour",
