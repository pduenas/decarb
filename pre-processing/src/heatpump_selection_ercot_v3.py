...
def get_capacity_at_design_temp(
    ...
    if cooling_series.isna().all() or cooling_series.max() == 0:
        ...
        cooling_temp_f = consumption_df["out.outdoor_air_drybulb_temp..c"].mean() * 1.8 + 32
        if verbose:
    ...
    return result, peak_load_btu_hr, peak_cooling_btu_hr, outdoor_temp_f
...
def populate_heatpump_metrics(
    ...
    cap_47 = row["Rated Capacity 47°F"]
    caps = np.array([
...
if __name__ == "__main__":
    ...
    for filepath in consumption_files:
        ...
        try:
            ...
            neep_df, peak_load_btu_hr, peak_cooling_btu_hr, outdoor_temp_f_val = get_capacity_at_design_temp(neep_base, consumption_df, ccASHP_df=ccASHP_df, conditioned_area_sqft=building["conditioned_area_sqft"], verbose=False)
            ...
            if n_sufficient == 0:
                ...
                if neep_df is None:
                    ...
                    metrics = populate_heatpump_metrics(neep_df, cheapest.name, building_id, peak_load_btu_hr, outdoor_temp_f_val, cheapest["cost_total"], consumption_df)
                    metrics["peak_load_btu_hr"] = peak_load_btu_hr
                    metrics["outdoor_temp_f"] = round(outdoor_temp_f_val, 1)
