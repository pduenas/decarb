    ...
    with filepath.open(newline="", encoding="utf-8") as fh:
        ...
        for lineno, row in enumerate(reader, start=2):
            ...
            rows.append({
                ...
                "k2":        parse_float(row["k2"]),
                "k3":        parse_float(row["k3"]),
...
def build_mapping(csv_path: Path) -> dict:
    """
    Read CSV and return bldg_id -> {k1, k2, k3, mse, r_squared}.
    Duplicate bldg_ids are resolved by keeping the entry with the lower MSE
    (None MSE is treated as worse than any real value).
    """
    ...
    for row in rows:
        bid   = row["bldg_id"]
        entry = {k: row[k] for k in ("k1", "k2", "k3", "mse", "r_squared")}

        if bid not in mapping:
            mapping[bid] = entry
            continue

        existing_mse = mapping[bid]["mse"]
        new_mse      = entry["mse"]
        if new_mse is not None and (existing_mse is None or new_mse < existing_mse):
            print(f"  [WARNING] Duplicate bldg_id={bid}. Replacing with lower-MSE entry.")
            mapping[bid] = entry
...
def print_and_collect_stats(mapping: dict, label: str) -> list[dict]:
    ...
    metrics = {
        ...
        "mse":       [e["mse"]       for e in mapping.values() if e["mse"]       is not None],
    ...
    for metric, values in metrics.items():
        ...
        display = "R²" if metric == "r_squared" else "MSE" if metric == "mse" else metric
...
def plot_histograms(mapping: dict, output_dir: Path) -> None:
    ...
    Produce two PNG files:
      ...
      histogram_fit.png          — MSE and R² side by side
    """
...
def pick_sample_buildings(
    ...
    Returns a list of tuples: (bldg_id, r2, mse, y_true, y_pred)
    """
    candidates = [
    ...
    for bid in pool:
        ...
        sample.append((bid, e["r_squared"], e["mse"], df["indoor_temp_c"], y_pred))
...
def make_sample_figure(
    ...
    for ax, (bid, r2, mse, y_true, y_pred) in zip(axes, sample):
        ...
        ax.set_title(f"Building {bid}  |  R² = {r2:.3f}  |  MSE = {mse:.4f}", fontsize=10)
