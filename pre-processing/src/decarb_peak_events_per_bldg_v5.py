...
def _is_tou_case(case_name):
    """True only for TOU cases — flat and flat+cap return False."""
    name = case_name.lower()
    if "cap" in name or "flat" in name:
        return False
    return "tou" in name
...
def _title_from_case_name(case_name):
    ...
    Tariff:
      - 'cap'  → 'Flat Tariff w/ CC'   (checked first since cap names also contain 'flat')
      - 'flat' → 'Flat Tariff'
      - 'tou'  → 'TOU Tariff'
    Electrification:
