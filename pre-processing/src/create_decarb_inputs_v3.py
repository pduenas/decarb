...
def create_sp_file(output_dir: Path, template: pd.DataFrame,
    ...
    if "Shared Heating" in heating_id:
        ...
        # valid_heating = [t for t in hvac_df["ty"].tolist() + chp_df["ty"].tolist()
        #          if t and str(t).lower() != "none"
        #          and (hvac_df.loc[hvac_df['ty'] == t, 'pHVmx'].sum() > 0 
        #               or chp_df.loc[chp_df['ty'] == t, 'pHVmx'].sum() > 0)]
    ...
    # print("hvac_df columns:", hvac_df.columns.tolist())
    # print("chp_df columns:", chp_df.columns.tolist())
    ...
    #     cooling_id = chosen
    if cooling_shared:
    # Filter to systems that actually have cooling capacity
        valid_cooling_df = hvac_df[
        (hvac_df["ty"].notna()) &
        (hvac_df["ty"].astype(str).str.lower() != "none") &
        (hvac_df["pACmx"] > 0)]   # must be able to cool
    
        valid_cooling = valid_cooling_df["ty"].tolist()

        if not valid_cooling:
            raise ValueError(f"No cooling-capable HVAC options for building {bldg_id}")

        chosen = rng.choice(valid_cooling)

        print(f"  WARNING bldg {bldg_id}: cooling '{cooling_id}' → cooling-capable '{chosen}'")
        cooling_id = chosen
...
def _worker_init(parent_path: Path, metadata_df, regression_coeffs: dict,
    ...
    _worker_state["metadata_df"]       = metadata_df
    ...
    _worker_state["hp_summary_df"]     = hp_summary_df
