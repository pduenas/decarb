"""
chp_units!(model::Model,in::Dict,tm::Dict,bdg::Dict,topo::Dict,sp::Dict,chp::Dict,
    abs::Dict)
...
"""
function chp_units!(model::Model,in::Dict,tm::Dict,bdg::Dict,topo::Dict,sp::Dict,
    chp::Dict,abs::Dict)
    ...
    @variable(model, bCHPty[i=1:in["IT"],c=1:chp["N"]], Bin)
    ...
    chp["mx"][iszero.(upper_bound.(bCHPty[1,:]))] .= 0

    # energy --electricity-- generated by CHP [kWh]
    @expression(model, vCHP_Q[t=1:tm["P"],c=1:chp["N"]], tm["TM"][t]*chp["mx"][c]*vCHP_q[t,c])
    ...
    @expression(model, vCHP_HT[t=1:tm["P"],c=1:chp["N"]; (!iszero).(topo["chp_bdg"][c,1])],
        tm["TM"][t] * (chp["mx"][c]*topo["chp_bdg"][c,1]*model[:vCHPbdg][t,c,1] +
                       sum(topo["chp_fire"][c,abs["N"]+1,f]*model[:vCHPfire][t,c,abs["N"]+1,f] for f=1:2)))
    ...
    @constraint(model, eCHPbdg,
        sum(bCHPty[i,c] for i=1:in["IT"],c=1:chp["N"] if chp["mx"][c]>0) <= bdg["Bchp"])
    # only one investment per time window for CHP units {0,1}
    @constraint(model, eCHPw[c=1:chp["N"]; chp["mx"][c]>0],
        sum(bCHPty[i,c] for i=1:in["IT"]) <= 1)
    # maximum electricity provided by CHP [0,1]
    @constraint(model, eCHPmx[t=1:tm["P"],c=1:chp["N"]; chp["mx"][c]>0],
        vCHP_q[t,c] <= bCHP_q[t,c])
    # minimum electricity provided by CHP [0,1]
    @constraint(model, eCHPmn[t=1:tm["P"],c=1:chp["N"]; chp["mx"][c]>0 && chp["mn"][c]>0],
    	vCHP_q[t,c] >= chp["mn"][c]*bCHP_q[t,c])
    # allowed commitment of CHP unit when existing {0,1}
    @constraint(model, eCHPu[t=1:tm["P"],c=1:chp["N"]; chp["mx"][c]>0],
    ...
    @constraint(model, eCHPht[t=1:tm["P"],c=1:chp["N"]; chp["mx"][c]>0 && chp["h2p"][c]>0],
        sum(model[:vCHPbdg][t,c,l] for l=1:2 if topo["chp_bdg"][c,l]>0) +
        sum(model[:vCHPabs][t,c,a] for a=1:abs["N"] if topo["chp_abs"][c,a]>0) <= chp["h2p"][c]*vCHP_q[t,c])
    # maximum heat provided by CHP [0,1] - boilers
    @constraint(model, eCHPht0[t=1:tm["P"],c=1:chp["N"]; chp["mx"][c]>0 && chp["h2p"][c]==0],
        sum(model[:vCHPbdg][t,c,l] for l=1:2 if topo["chp_bdg"][c,l]>0) +
        sum(model[:vCHPabs][t,c,a] for a=1:abs["N"] if topo["chp_abs"][c,a]>0) <= bCHP_u[t,c])
