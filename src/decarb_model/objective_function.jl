...
function objective_function!(model::Model,cfg::Dict,tm::Dict,chp::Dict,abp::Dict,hvac::Dict,
    ...
    @expression(model, vNSEVcost[t=1:tm["P"]],
        cfg["NSEVcost"]*sum(model[:vEVlo][t,e] for e=1:ev["N"]; init=0.0))
    ...
    @expression(model, vEVpen,
        sum(cfg["EVdriver"]*cfg["EVpen"]*model[:vEVsoc][t,e] for t=1:tm["P"],e=1:ev["N"]; init=0.0))
    
    # cost of peak capacity subscription [$]
    @expression(model, vQmxCost, sum(tm["QmxCostN"][n]*model[:vQmx][n] for n=1:cfg["QmxTM"]; init=0.0))
    ...
    @expression(model, vCHPinv,
        ...
        (1-(1+cfg["IR"])^(-chp["life"][c]))*model[:bCHPty][i,c] for i=1:cfg["IT"],c=1:chp["N"]; init=0.0))
    ...
    @expression(model, vHVACinv,
        ...
        (1-(1+cfg["IR"])^(-hvac["life"][h]))*model[:bHVACty][i,h] for i=1:cfg["IT"],h=1:hvac["N"]; init=0.0))
    ...
    @expression(model, vABPinv, 
        ...
        (1-(1+cfg["IR"])^(-abp["life"][a]))*model[:bABPty][i,a] for i=1:cfg["IT"],a=1:abp["N"]; init=0.0))
    ...
    @expression(model, vWHinv, 
        ...
        (1-(1+cfg["IR"])^(-wh["life"][w]))*model[:bWHty][i,w] for i=1:cfg["IT"],w=1:wh["N"]; init=0.0))
    ...
    b = has_upper_bound.(model[:zPV])       # for potential investments
        ...
        (1-(1+cfg["IR"])^(-pv["life"][v]))*model[:zPV][i,v] for i=1:cfg["IT"],v=1:pv["N"]; init=0.0))
    ...
    b = has_upper_bound.(model[:zWIND])     # for potential investments
        ...
        (1-(1+cfg["IR"])^(-wind["life"][d]))*model[:zWIND][i,d] for i=1:cfg["IT"],d=1:wind["N"]; init=0.0))
    ...
    b = has_upper_bound.(model[:zBESS])     # for potential investments
        ...
        (1-(1+cfg["IR"])^(-bess["life"][s]))*model[:zBESS][i,s] for i=1:cfg["IT"],s=1:bess["N"]; init=0.0))

    # total fixed O&M costs (+) [$]
    @expression(model, COST_FOM,
        sum(chp["fom"][c]*model[:bCHPty][i,c]*tm["H"]/8760 for i=1:cfg["IT"],c=1:chp["N"]; init=0.0) +
    	sum(abp["fom"][a]*model[:bABPty][i,a]*tm["H"]/8760 for i=1:cfg["IT"],a=1:abp["N"]; init=0.0) +
    	sum(hvac["fom"][h]*model[:bHVACty][i,h]*tm["H"]/8760 for i=1:cfg["IT"],h=1:hvac["N"]; init=0.0) +
    	sum(wh["fom"][w]*model[:bWHty][i,w]*tm["H"]/8760 for i=1:cfg["IT"],w=1:wh["N"]; init=0.0) +
    	sum(pv["fom"][v]*model[:zPV][i,v]*tm["H"]/8760 for i=1:cfg["IT"],v=1:pv["N"]; init=0.0) +
    	sum(wind["fom"][d]*model[:zWIND][i,d]*tm["H"]/8760 for i=1:cfg["IT"],d=1:wind["N"]; init=0.0) +
    	sum(bess["fom"][s]*model[:zBESS][i,s]*tm["H"]/8760 for i=1:cfg["IT"],s=1:bess["N"]; init=0.0))
    # total variable O&M costs (+) [$]
    @expression(model, COST_VOM,
        sum(chp["vom"][c]*model[:vCHP_Q][t,c] for t=1:tm["P"],c=1:chp["N"]; init=0.0) +
        sum(abp["vom"][a]*model[:vABP_AC][t,a] for t=1:tm["P"],a=1:abp["N"]; init=0.0) +
        sum(hvac["vom"][h]*model[:vHVAC_HTAC][t,h] for t=1:tm["P"],h=1:hvac["N"]; init=0.0) +
        sum(wh["vom"][w]*model[:vWH_HW][t,w] for t=1:tm["P"],w=1:wh["N"]; init=0.0))
