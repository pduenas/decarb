"""
water_heaters!(model::Model,in::Dict,tm::Dict,bdg::Dict,topo::Dict,sp::Dict,wh::Dict)
...
"""
function water_heaters!(model::Model,in::Dict,tm::Dict,bdg::Dict,sp::Dict,wh::Dict)
    ...
    @variable(model, bWHty[i=1:in["IT"],w=1:wh["N"]], Bin)
    ...
    if any(wh["fuel"].=="G")
        @expression(model, vWH_G[t=1:tm["P"]], sum(wh["hr"][w]*vWH_Q[t,w] for w=1:wh["N"]
    ...
    if any(wh["fuel"].=="L")
        @expression(model, vWH_L[t=1:tm["P"]], sum(wh["hr"][w]*vWH_Q[t,w] for w=1:wh["N"]
    ...
    @constraint(model, eWHbal[t=1:tm["P"],w=1:wh["N"]; wh["mx"][w]>0],
    	...
    	sum(in["WHsto0"][1]*bWHty[i,w] for i=1:in["IT"] if t==tm["IW"][i] && wh["tank"][w]>0))
    # fix final tank level [0,1]
    @constraint(model, eWHf[w=1:wh["N"]; wh["tank"][w]>0],
        vWHsoc[tm["P"],w] == in["WHstof"][1]*bWH_u[tm["P"],w])
