...
function electric_load!(model::Model,in::Dict,tm::Dict,chp::Dict,hvac::Dict,wh::Dict,
    ...
    @expression(model, vQdem[t=1:tm["P"]],
        tm["TM"][t]*(tm["Qlight"][t]+tm["Qequip"][t]) + sum(model[:vHVAC_HT][t,h]+model[:vHVAC_AC][t,h] for h=1:hvac["N"]) + 
        sum(model[:vBESS_UP][t,s] for s=1:bess["N"] if bess["zmx0"][s]>0) + sum(vWH_Q[t,w] for w=1:wh["N"] if wh["fuel"][w]==""))
