...
function run_decarb!(path::AbstractString,mip_gap::Float64,time_limit::Float64,i_solver::Int64,b_relax_integrality::Bool)
    ...
    df_elec,balance = read_electric(model,tm,chp["N"],hvac["N"],wh["N"],pv["N"],
        bess["N"],ev["N"],wind["N"],bess["mx"],bess["zmx0"],ev["mx_eff"],collect(wh["fuel"]))
    df_fuel = read_fuel(model,tm,chp["N"],abp["N"],wh["N"])
    df_indoor = read_indoor(model,tm,chp["N"],abp["N"],hvac["N"],wh["N"],wh["tank_eff"],
        topo["chp_bdg"])
